...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<ApiResponse<AuthDto>> signup(@Valid @RequestBody SignupRequest signupRequest) {
        AuthDto authDto = authService.signup(signupRequest);
        return ResponseEntity.status(HttpStatus.CREATED).body(new ApiResponse<>("회원가입 성공", authDto));
    }

    @Operation(summary = "로그인", description = "이메일과 비밀번호를 입력 받아 로그인합니다.")
    @PostMapping("/login")
    public ResponseEntity<ApiResponse<AuthDto>> login(@Valid @RequestBody LoginRequest loginRequest) {
        AuthDto authDto = authService.login(loginRequest);
        return ResponseEntity.ok(new ApiResponse<>("로그인 성공", authDto));
    }
